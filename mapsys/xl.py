...
def write_dxf_report(
    report_rows: list[dict[str, Any]],
    report_path: Path,
) -> None:
    """Write a DXF conversion report to an Excel file.

    Creates a workbook with a single "Conversion report" sheet containing
    one row per converted file: hyperlinks to mapsys and DXF paths, Content
    list counts, generation timestamp, and DXF file size.

    Args:
        report_rows: List of dicts with keys: mapsys_path, dxf_path,
            mapsys_name, points, texts, t_meta, p_meta, v_offsets, p_layers,
            pr5_layers, pr5_after, pr5_fonts, generated_at, dxf_size_bytes.
    ...
    for row_idx, row in enumerate(report_rows, start=2):
        ...
        ws.cell(
            row=row_idx,
            column=col_map["DXF size (bytes)"],
            value=row["dxf_size_bytes"],
        )
        ...
        # DXF path with hyperlink.
        dxf_cell = ws.cell(row=row_idx, column=dxf_col, value=row["dxf_path"])
        dxf_cell.hyperlink = str(Path(row["dxf_path"]).as_uri())
